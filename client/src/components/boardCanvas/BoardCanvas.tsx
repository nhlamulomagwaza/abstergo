...
const BoardCanvas: React.FC = () => {
  const { selectedBoard, boardOptionsOpen, setBoardOptionsOpen,
  ...
  const { addTask, updateTask } = useContext(AbstergoTasksContext) as TasksContextTypes;
  ...
  useEffect(() => {
    ...
    fetchSingleBoard();
  }, [selectedBoard, addTask, updateTask]);
